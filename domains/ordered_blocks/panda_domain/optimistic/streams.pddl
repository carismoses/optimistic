(define (stream ordered-blocks-optimistic-panda)
  ...
  (:stream plan-holding-motion
    :inputs (?q1 ?q2 ?b ?g)
    :domain (and (Conf ?q1) (Conf ?q2) (Block ?b) (Grasp ?g))
    :fluents (AtPose)
    :outputs (?t)
  ...
  (:stream pick-inverse-kinematics
    :inputs (?b ?p ?g)
    :domain (and (Block ?b) (Pose ?b ?p) (Grasp ?g))
    :outputs (?q1 ?q2 ?t)
    :certified (and (Conf ?q1) (Conf ?q2) (PickKin ?b ?p ?g ?q1 ?q2 ?t))
  )
  ...
  (:stream place-inverse-kinematics
    ...
    :domain (and (Block ?b) (Pose ?b ?p) (Grasp ?g))
    :outputs (?q1 ?q2 ?t)
    :certified (and (Conf ?q1) (Conf ?q1) (PlaceKin ?b ?p ?g ?q1 ?q2 ?t))
  )
